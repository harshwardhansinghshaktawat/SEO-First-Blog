class BlogListViewer extends HTMLElement {
        ...
        return `
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .blog-grid {
                ...
                gap: 32px;
                margin-bottom: 60px;
                /* Performance hint for rendering */
    ...
    _renderPosts() {
        ...
        grid.innerHTML = this._posts.map((post, index) => {
            // Optimize images with proper dimensions matching card size
            // Cards are max 340px wide, images are 240px tall
            // First 6 images (2 rows) get high priority for LCP
            const isAboveFold = index < 6;
            
            // Featured image: card width ~340-400px, height 240px
            // Use 2x resolution for retina: 680x480
            const featuredImageUrl = this._convertWixImageUrl(post.featuredImage, {
                width: 680,
                height: 480,
                quality: isAboveFold ? 80 : 75,  // Lower quality, still looks good
                operation: 'fill'
            });
            
            // Author images: displayed at 40px, use 80px for retina
