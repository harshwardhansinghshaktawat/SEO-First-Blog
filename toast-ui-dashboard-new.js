...
class MdxBlogEditor extends HTMLElement {
    ...
    attributeChangedCallback(name, oldVal, newVal) {
        ...
        try {
            ...
            console.error('[MdxEditor] Error:', name, e); 
        }
    }
        ...
        requestAnimationFrame(() => {
            ...
            if (this._pendingPostList) {
                try {
                    ...
                    console.error('[MdxEditor] Error:', e);
                }
            }
...
mdx-blog-editor .mdx-tab-bar::-webkit-scrollbar { height: 3px; }
mdx-blog-editor .mdx-tab-bar::-webkit-scrollbar-thumb { background: var(--border); }
...
mdx-blog-editor .mdx-search-section {
    display: flex;
    gap: 10px;
    margin-bottom: 20px;
...
mdx-blog-editor .mdx-search-input {
    flex: 1;
    padding: 10px 14px;
    border: 1.5px solid var(--border);
    border-radius: var(--r);
...
mdx-blog-editor .mdx-search-input:focus {
    border-color: var(--accent);
}

mdx-blog-editor .mdx-selected-related {
...
mdx-blog-editor .mdx-post-results {
    display: flex;
    flex-direction: column;
    gap: 8px;
    padding: 0 20px;
...
mdx-blog-editor .mdx-post-item:hover {
    border-color: var(--accent);
    background: #fff5f0;
}

mdx-blog-editor .mdx-post-item.selected {
    background: var(--green);
    border-color: var(--green);
    color: #fff;
}

mdx-blog-editor .mdx-post-item-title {
    font-weight: 600;
    font-size: 14px;
    margin-bottom: 4px;
}

mdx-blog-editor .mdx-post-item-excerpt {
    font-size: 12px;
    color: var(--ink3);
}

mdx-blog-editor .mdx-post-item.selected .mdx-post-item-excerpt {
    color: rgba(255,255,255,0.8);
}

mdx-blog-editor .mdx-internal-link-item {
    padding: 14px;
    background: #fff;
    border: 1px solid var(--border);
    border-radius: var(--r);
...
mdx-blog-editor .mdx-internal-link-header {
    display: flex;
    justify-content: space-between;
    align-items: center;
    margin-bottom: 10px;
...
mdx-blog-editor .mdx-internal-link-phrase {
    font-weight: 600;
    font-size: 14px;
    color: var(--accent);
}

mdx-blog-editor .mdx-internal-link-target {
    font-size: 13px;
    color: var(--ink2);
}

mdx-blog-editor .mdx-internal-link-form {
    display: grid;
    gap: 12px;
    padding: 14px;
    background: var(--paper2);
    ...
    <div class="mdx-editor-body">
        <div class="mdx-editor-main">
            ...
            <div class="mdx-related-panel" id="relatedPanel">
                <div class="mdx-meta-inner">
                    <div class="mdx-search-section">
                        <input type="text" 
                               class="mdx-search-input" 
                               id="relatedSearchInput" 
                               placeholder="Search posts...">
                        <button class="mdx-btn mdx-btn-light mdx-btn-sm" id="relatedSearchBtn">${this._icon('search')}</button>
                    </div>
                    <div class="mdx-selected-related" id="selectedRelated"></div>
                    <div class="mdx-post-results" id="relatedResults"></div>
                </div>
            </div>

            <div class="mdx-internal-panel" id="internalPanel">
                <div class="mdx-meta-inner">
                    <div class="mdx-msec">
                        <div class="mdx-msec-title">Auto Internal Linking</div>
                        <div class="mdx-mfields">
                            <div class="mdx-mfield mdx-mfull">
                                <button class="mdx-btn mdx-btn-accent" id="addInternalLinkBtn">${this._icon('plus')} Add Link Phrase</button>
                            </div>
                        </div>
                        <div id="internalLinksForm" style="display:none;" class="mdx-internal-link-form">
                            <input type="text" class="mdx-minp" id="linkPhraseInput" placeholder="Enter phrase (e.g., 'SEO best practices')">
                            <select class="mdx-msel" id="linkTargetSelect">
                                <option value="">Select target post...</option>
                            </select>
                            <div style="display:flex;gap:8px;">
                                <button class="mdx-btn mdx-btn-accent mdx-btn-sm" id="saveLinkBtn">${this._icon('check')} Save</button>
                                <button class="mdx-btn mdx-btn-light mdx-btn-sm" id="cancelLinkBtn">${this._icon('close')} Cancel</button>
                            </div>
                        </div>
...
<div class="mdx-msec">
    ...
    <div class="mdx-mfields">
        ...
        <div class="mdx-mfield mdx-mfull"><label>Tags</label><input class="mdx-minp" type="text" placeholder="web, tech, javascript" data-m="tags"></div>
...
<div class="mdx-msec">
    ...
    <div class="mdx-mfields">
        ...
        <div class="mdx-mfield mdx-mfull"><label>Keywords</label><input class="mdx-minp" type="text" placeholder="keyword1, keyword2" data-m="seoKeywords"></div>
    </div>
</div>
<div class="mdx-msec">
  ...
  _wireRelatedPosts() {
    ...
    _wireInternalLinks() {
        const addBtn = this.querySelector('#addInternalLinkBtn');
        const form = this.querySelector('#internalLinksForm');
        const saveBtn = this.querySelector('#saveLinkBtn');
        const cancelBtn = this.querySelector('#cancelLinkBtn');
        const phraseInput = this.querySelector('#linkPhraseInput');
        const targetSelect = this.querySelector('#linkTargetSelect');
        ...
        saveBtn.addEventListener('click', () => {
            ...
            if (!phrase || !targetId) {
                this._toast('error', 'Please fill in both fields');
                return;
            }
            
            const targetPost = this._allPosts.find(p => p._id === targetId);
            if (!targetPost) return;
            
            if (!this._meta.internalLinks) {
                this._meta.internalLinks = [];
            }
            
            this._meta.internalLinks.push({
                phrase: phrase,
                targetId: targetId,
                targetSlug: targetPost.slug,
                targetTitle: targetPost.blogTitle || targetPost.title
            });
            
            this._renderInternalLinks();
            this._applyInternalLinks();
            
            form.style.display = 'none';
            phraseInput.value = '';
            targetSelect.value = '';
            
            this._toast('success', 'Internal link added!');
        });
    }

    _populateTargetSelect() {
        const select = this.querySelector('#linkTargetSelect');
        if (!select) return;
        
        select.innerHTML = '<option value="">Select target post...</option>';
        
        this._allPosts.forEach(post => {
            if (post._id === this._editPost?._id) return;
            const option = document.createElement('option');
            option.value = post._id;
            option.textContent = post.blogTitle || post.title || 'Untitled';
            select.appendChild(option);
        });
    }

    _renderInternalLinks() {
        const container = this.querySelector('#internalLinksList');
        if (!container) return;
        ...
        container.innerHTML = links.map((link, idx) => `
            <div class="mdx-internal-link-item">
                <div class="mdx-internal-link-header">
                    <div class="mdx-internal-link-phrase">"${link.phrase}"</div>
                    <button class="mdx-btn mdx-btn-red mdx-btn-sm" data-idx="${idx}">${this._icon('trash')}</button>
                </div>
                <div class="mdx-internal-link-target">→ ${link.targetTitle}</div>
            </div>
        `).join('');
        ...
        container.querySelectorAll('button').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                this._meta.internalLinks.splice(idx, 1);
                this._renderInternalLinks();
                this._applyInternalLinks();
                this._toast('success', 'Link removed');
            });
        });
    }
    ...
    _applyInternalLinks() {
        ...
        let content = this._toastEditor.getMarkdown();
        const links = this._meta.internalLinks || [];
        
        links.forEach(link => {
            const regex = new RegExp(`\\b(${link.phrase})\\b`, 'gi');
            const replacement = `[$1](/${link.targetSlug})`;
    ...
    _renderSearchResults() {
        const container = this.querySelector('#relatedResults');
        if (!container) return;
        
        const selected = this._meta.relatedPosts || [];
        
        if (this._searchResults.length === 0) {
            container.innerHTML = '<p style="padding:14px;color:var(--ink3);font-size:13px;">No posts found</p>';
            return;
        }
        
        container.innerHTML = this._searchResults.map(post => {
            const isSelected = selected.includes(post._id);
            return `
                <div class="mdx-post-item ${isSelected ? 'selected' : ''}" data-id="${post._id}">
                    <div class="mdx-post-item-title">${post.blogTitle || post.title || 'Untitled'}</div>
                    <div class="mdx-post-item-excerpt">${post.excerpt || 'No excerpt'}</div>
                </div>
            `;
        }).join('');
        
        container.querySelectorAll('.mdx-post-item').forEach(item => {
            item.addEventListener('click', () => {
                const id = item.dataset.id;
                this._toggleRelatedPost(id);
            });
        });
        
        this._renderSelectedRelated();
    }

    _toggleRelatedPost(id) {
        if (!this._meta.relatedPosts) {
            this._meta.relatedPosts = [];
        }
        
        const idx = this._meta.relatedPosts.indexOf(id);
        if (idx > -1) {
    ...
    _renderSelectedRelated() {
        const container = this.querySelector('#selectedRelated');
        if (!container) return;
        
        const selected = this._meta.relatedPosts || [];
        
        if (selected.length === 0) {
            container.innerHTML = '<p style="padding:14px 0;color:var(--ink3);font-size:13px;">No related posts selected</p>';
            return;
        }
        
        const selectedPosts = selected.map(id => {
            const post = this._allPosts.find(p => p._id === id);
            return post ? post.blogTitle || post.title || 'Untitled' : 'Unknown';
        });
        
        container.innerHTML = `
            <p style="font-size:12px;color:var(--ink3);margin-bottom:8px;">Selected (${selected.length}):</p>
            ${selectedPosts.map((title, idx) => `
                <span class="mdx-related-tag">
                    ${title}
                    <span class="mdx-related-remove" data-idx="${idx}">×</span>
                </span>
            `).join('')}
        `;
        
        container.querySelectorAll('.mdx-related-remove').forEach(span => {
    ...
    async _loadToastEditor() {
        ...
        try {
            ...
            script.onerror = () => {
                ...
                this._toast('error', 'Failed to load editor');
            };
    ...
    _onPostList(data) {
        ...
        if (!this._posts.length) {
            content.innerHTML = `<div class="mdx-state-box">${this._icon('image')}<p>No posts yet. Click "New Post"!</p></div>`;
            return;
        }
...
<td>
    ...
    _switchTab(tab) {
        ...
        if (tab === 'related') this._renderSearchResults();
        if (tab === 'internal') {
            this._renderInternalLinks();
            this._populateTargetSelect();
        }
    ...
    _runSEOAnalysis() {
        ...
        if (keyphrase && blogTitle.toLowerCase().includes(keyphrase.toLowerCase())) {
            score += 15;
            checks.push({ status: 'good', text: 'Keyphrase in blog title' });
        } else if (keyphrase) {
        ...
        if (keyphrase && seoTitle.toLowerCase().includes(keyphrase.toLowerCase())) {
            ...
            checks.push({ status: 'ok', text: 'Consider keyphrase in SEO title' });
        }
        ...
        if (seoTitle.length >= 50 && seoTitle.length <= 60) {
            ...
            score += 5;
            checks.push({ status: 'ok', text: `SEO title: ${seoTitle.length} chars (aim 50-60)` });
        ...
        if (seoDesc.length >= 120 && seoDesc.length <= 160) {
            score += 15;
            checks.push({ status: 'good', text: `Meta description optimal (${seoDesc.length})` });
        } else if (seoDesc.length > 0 && seoDesc.length < 170) {
            score += 8;
            checks.push({ status: 'ok', text: `Meta desc: ${seoDesc.length} (aim 120-160)` });
        } else if (seoDesc.length > 170) {
            checks.push({ status: 'bad', text: `Meta desc too long (${seoDesc.length})` });
        } else {
            checks.push({ status: 'bad', text: 'Add meta description (120-160)' });
        }
        ...
        if (keyphrase && seoDesc.toLowerCase().includes(keyphrase.toLowerCase())) {
            ...
            checks.push({ status: 'bad', text: 'Add keyphrase to meta desc' });
        }
        ...
        if (keyphrase && content) {
            ...
            if (density >= 0.5 && density <= 2.5) {
                ...
                score += 5;
                checks.push({ status: 'ok', text: `More keyphrase use (${density.toFixed(1)}%)` });
            } else if (density > 2.5) {
        ...
        if (hasHeadings) {
            ...
            checks.push({ status: 'bad', text: 'Add headings to structure content' });
        }
    ...
    _runReadabilityAnalysis() {
        ...
        if (avgSentenceLength <= 20) {
            score += 25;
            checks.push({ status: 'good', text: `Sentences easy (avg ${avgSentenceLength.toFixed(1)} words)` });
        } else if (avgSentenceLength <= 25) {
            score += 15;
            checks.push({ status: 'ok', text: `Sentence length OK (avg ${avgSentenceLength.toFixed(1)})` });
        } else {
            score += 5;
            checks.push({ status: 'bad', text: `Sentences long (avg ${avgSentenceLength.toFixed(1)})` });
        }
        ...
        if (paragraphRatio === 0) {
            ...
            checks.push({ status: 'bad', text: 'Paragraphs too long (<150 words)' });
        }
        ...
        if (transitionCount >= 3) {
            score += 15;
            checks.push({ status: 'good', text: 'Good use of transitions' });
        } else if (transitionCount > 0) {
            score += 8;
            checks.push({ status: 'ok', text: 'Use more transition words' });
        } else if (wordCount > 200) {
            checks.push({ status: 'bad', text: 'Add transition words' });
        }
