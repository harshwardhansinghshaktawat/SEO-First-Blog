class MdxBlogEditor extends HTMLElement {
    constructor() {
        ...
        this._highlightedIssueType = null;
    ...
    async _loadGrammarLibraries() {
        ...
        if (!window.compromiseSentences) {
            const script3 = document.createElement('script');
            script3.src = 'https://cdn.jsdelivr.net/npm/compromise-sentences@0.3.0/builds/compromise-sentences.min.js';
            script3.onload = () => {
                if (window.nlp && window.compromiseSentences) {
                    try { window.nlp.plugin(window.compromiseSentences); } catch(e) {}
                }
            };
    ...
    _icon(k) {
        const I = {
            ...
            highlight: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 1 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>`,
            spotify: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M8 15c3-1 6-1 9 1" stroke-linecap="round"/><path d="M7 12c4-1.5 8-1.5 12 1" stroke-linecap="round"/><path d="M6.5 9c5-2 10-2 15 1" stroke-linecap="round"/></svg>`,
            music: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg>`,
            embed: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>`,
            ...
            codepen: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="12 2 22 8.5 22 15.5 12 22 2 15.5 2 8.5 12 2"/><line x1="12" y1="22" x2="12" y2="15.5"/><polyline points="22 8.5 12 15.5 2 8.5"/><polyline points="2 15.5 12 8.5 22 15.5"/><line x1="12" y1="2" x2="12" y2="8.5"/></svg>`,
...
mdx-blog-editor .mdx-btn:disabled { opacity: .5; cursor: not-allowed; pointer-events: none; }

mdx-blog-editor .mdx-progress-overlay {
    position: fixed;
...
mdx-blog-editor .mdx-progress-overlay.active {
    opacity: 1;
    pointer-events: all;
}
mdx-blog-editor .mdx-progress-card {
    background: #fff;
    border-radius: 12px;
    padding: 28px 36px;
    min-width: 340px;
...
mdx-blog-editor .mdx-progress-title {
    font-size: 16px;
    font-weight: 700;
    margin-bottom: 6px;
    color: var(--ink);
}
mdx-blog-editor .mdx-progress-subtitle {
    font-size: 13px;
    color: var(--ink3);
    margin-bottom: 18px;
}
mdx-blog-editor .mdx-progress-track {
    width: 100%;
    height: 8px;
...
mdx-blog-editor .mdx-progress-fill {
    height: 100%;
...
mdx-blog-editor .mdx-progress-percent {
    font-size: 13px;
    font-weight: 600;
    color: var(--accent);
}

mdx-blog-editor .mdx-inline-progress {
    display: none;
    align-items: center;
    gap: 10px;
    padding: 8px 14px;
...
mdx-blog-editor .mdx-inline-progress.active { display: flex; }
mdx-blog-editor .mdx-inline-progress-track {
    flex: 1;
    height: 5px;
    background: #fff3cd;
    border-radius: 3px;
...
mdx-blog-editor .mdx-inline-progress-fill {
    height: 100%;
...
mdx-blog-editor .mdx-hl-passive {
    background: rgba(255, 100, 100, 0.22) !important;
    border-bottom: 2px wavy #cf1322 !important;
    border-radius: 2px;
    padding: 1px 0;
    transition: background .2s;
}
mdx-blog-editor .mdx-hl-longsentence {
    background: rgba(250, 140, 22, 0.2) !important;
    border-bottom: 2px dashed #fa8c16 !important;
    border-radius: 2px;
    padding: 1px 0;
    transition: background .2s;
}
mdx-blog-editor .mdx-hl-complex {
    background: rgba(138, 43, 226, 0.15) !important;
    border-bottom: 2px dotted #722ed1 !important;
    border-radius: 2px;
    padding: 1px 0;
    transition: background .2s;
}
mdx-blog-editor .mdx-hl-adverb {
    background: rgba(22, 119, 255, 0.18) !important;
    border-bottom: 2px dotted #1677ff !important;
    border-radius: 2px;
    padding: 1px 0;
    transition: background .2s;
}
mdx-blog-editor .mdx-hl-spelling {
    background: rgba(255, 0, 0, 0.2) !important;
    border-bottom: 2px wavy red !important;
    border-radius: 2px;
    padding: 1px 0;
}
mdx-blog-editor .mdx-hl-repeated {
    background: rgba(250, 173, 20, 0.2) !important;
    border-bottom: 2px dotted #faad14 !important;
    border-radius: 2px;
    padding: 1px 0;
}

mdx-blog-editor .mdx-highlight-btn {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    width: 22px;
...
mdx-blog-editor .mdx-highlight-btn:hover { opacity: 1; background: rgba(0,0,0,.06); }
mdx-blog-editor .mdx-highlight-btn.active { opacity: 1; background: var(--accent); }
mdx-blog-editor .mdx-highlight-btn.active svg { color: #fff !important; stroke: #fff !important; }
mdx-blog-editor .mdx-highlight-btn svg { width: 13px; height: 13px; }
...
mdx-blog-editor .mdx-analysis-text {
    flex: 1;
}
...
mdx-blog-editor .mdx-prev-inner .video-embed video {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
...
mdx-blog-editor .mdx-prev-inner .spotify-embed {
    margin: 16px 0;
    border-radius: 12px;
    overflow: hidden;
}

mdx-blog-editor .mdx-prev-inner .spotify-embed iframe {
    width: 100%;
    border: 0;
    border-radius: 12px;
}

mdx-blog-editor .mdx-prev-inner .twitter-embed,
mdx-blog-editor .mdx-prev-inner .codepen-embed,
mdx-blog-editor .mdx-prev-inner .generic-embed {
    margin: 16px 0;
    border-radius: var(--r);
    overflow: hidden;
}

mdx-blog-editor .mdx-prev-inner .generic-embed iframe {
    width: 100%;
    border: 0;
    border-radius: var(--r);
}
...
mdx-blog-editor .mdx-embed-dialog-overlay {
    position: fixed;
    top: 0; left: 0; right: 0; bottom: 0;
...
mdx-blog-editor .mdx-embed-dialog {
    background: #fff;
    border-radius: 12px;
    padding: 24px;
    min-width: 420px;
...
mdx-blog-editor .mdx-embed-dialog h3 {
    font-size: 18px;
    font-weight: 700;
    margin-bottom: 16px;
    color: var(--ink);
}
mdx-blog-editor .mdx-embed-dialog label {
    display: block;
    font-size: 12px;
    font-weight: 600;
    color: var(--ink3);
...
mdx-blog-editor .mdx-embed-dialog input,
mdx-blog-editor .mdx-embed-dialog select {
    width: 100%;
    padding: 9px 12px;
    border: 1.5px solid var(--border);
    border-radius: 5px;
...
mdx-blog-editor .mdx-embed-dialog input:focus,
mdx-blog-editor .mdx-embed-dialog select:focus {
    border-color: var(--accent);
}
mdx-blog-editor .mdx-embed-dialog-actions {
    display: flex;
    gap: 8px;
    justify-content: flex-end;
    margin-top: 8px;
}
mdx-blog-editor .mdx-embed-hint {
    font-size: 11px;
    color: var(--ink3);
    margin-bottom: 14px;
    line-height: 1.5;
...
@media (max-width: 900px) {
    ...
    mdx-blog-editor .mdx-embed-dialog { min-width: 300px; }
}
`; }
...
<div class="mdx-progress-overlay" id="progressOverlay">
    <div class="mdx-progress-card">
        <div class="mdx-progress-title" id="progressTitle">Processing...</div>
        <div class="mdx-progress-subtitle" id="progressSubtitle">Please wait</div>
        <div class="mdx-progress-track">
            <div class="mdx-progress-fill" id="progressFill"></div>
        </div>
        <div class="mdx-progress-percent" id="progressPercent">0%</div>
    </div>
</div>
...
<div class="mdx-msec">
    ...
    <div class="mdx-inline-progress" id="authorProgress">
        <span id="authorProgressLabel">Optimizing...</span>
        <div class="mdx-inline-progress-track"><div class="mdx-inline-progress-fill" id="authorProgressFill"></div></div>
    </div>
</div>
<div class="mdx-msec">
    ...
    <div class="mdx-inline-progress" id="featuredProgress">
        <span id="featuredProgressLabel">Optimizing...</span>
        <div class="mdx-inline-progress-track"><div class="mdx-inline-progress-fill" id="featuredProgressFill"></div></div>
    </div>
</div>`; }
...
<div class="mdx-msec">
    ...
    <div class="mdx-inline-progress" id="ogProgress">
        <span id="ogProgressLabel">Optimizing...</span>
        <div class="mdx-inline-progress-track"><div class="mdx-inline-progress-fill" id="ogProgressFill"></div></div>
    </div>
</div>`; }
    ...
    _wire() {
        ...
        this._wireImgZone('authorZone', 'authorFile', 'authorPrev', 'authorImage', 'authorProgress', 'authorProgressFill', 'authorProgressLabel');
        this._wireImgZone('featuredZone', 'featuredFile', 'featuredPrev', 'featuredImage', 'featuredProgress', 'featuredProgressFill', 'featuredProgressLabel');
        this._wireImgZone('ogZone', 'ogFile', 'ogPrev', 'seoOgImage', 'ogProgress', 'ogProgressFill', 'ogProgressLabel');
    ...
    _wireCategoryDropdown() {
        ...
        input.addEventListener('focus', () => { this._showCategoryDropdown(); });
        input.addEventListener('input', (e) => { this._meta.category = e.target.value; this._showCategoryDropdown(); });
        document.addEventListener('click', (e) => { if (!e.target.closest('.mdx-dropdown-wrapper')) { dropdown.style.display = 'none'; } });
    }
    ...
    _showCategoryDropdown() {
        ...
        let matchingCategories = this._allCategories.filter(cat => cat.name.toLowerCase().includes(searchTerm));
        dropdown.innerHTML = '';
        matchingCategories.forEach(cat => {
            const item = document.createElement('div');
            item.className = 'mdx-dropdown-item';
            item.textContent = cat.name;
            item.addEventListener('click', () => { input.value = cat.name; this._meta.category = cat.name; dropdown.style.display = 'none'; });
        ...
        if (searchTerm && !this._allCategories.some(cat => cat.name.toLowerCase() === searchTerm)) {
            ...
            createItem.addEventListener('click', () => {
                input.value = searchTerm; this._meta.category = searchTerm; dropdown.style.display = 'none';
                if (!this._newCategoriesCreated.includes(searchTerm)) { this._newCategoriesCreated.push(searchTerm); this._showNewItemsAlert(); }
            });
    ...
    _wireTagsDropdown() {
        ...
        if (!input || !dropdown) return;
        input.addEventListener('focus', () => { this._showTagsDropdown(); });
        input.addEventListener('input', (e) => { this._meta.tags = e.target.value; this._showTagsDropdown(); });
        document.addEventListener('click', (e) => { if (!e.target.closest('.mdx-dropdown-wrapper')) { dropdown.style.display = 'none'; } });
    }
    ...
    _showTagsDropdown() {
        ...
        const currentTag = lastCommaIndex >= 0 ? currentValue.substring(lastCommaIndex + 1).trim().toLowerCase() : currentValue.trim().toLowerCase();
        ...
        let matchingTags = this._allTags.filter(tag => tag.name.toLowerCase().includes(currentTag) && !existingTags.includes(tag.name.toLowerCase()));
        dropdown.innerHTML = '';
        matchingTags.forEach(tag => {
            ...
            item.addEventListener('click', () => {
                ...
                input.value = beforeLastComma + tag.name + ', '; this._meta.tags = input.value; dropdown.style.display = 'none'; input.focus();
            });
            dropdown.appendChild(item);
        });
        if (!this._allTags.some(tag => tag.name.toLowerCase() === currentTag)) {
            ...
            createItem.addEventListener('click', () => {
                const beforeLastComma = lastCommaIndex >= 0 ? currentValue.substring(0, lastCommaIndex + 1) + ' ' : '';
                input.value = beforeLastComma + currentTag + ', '; this._meta.tags = input.value; dropdown.style.display = 'none'; input.focus();
                if (!this._newTagsCreated.includes(currentTag)) { this._newTagsCreated.push(currentTag); this._showNewItemsAlert(); }
            });
            dropdown.appendChild(createItem);
        }
        dropdown.style.display = dropdown.children.length > 0 ? 'block' : 'none';
    ...
    _showNewItemsAlert() {
        ...
        if (newCats === 0 && newTags === 0) { alertContainer.innerHTML = ''; return; }
        let message = '<strong>New items will be created:</strong><br>';
        if (newCats > 0) { message += `• ${newCats} new categor${newCats > 1 ? 'ies' : 'y'}: ${this._newCategoriesCreated.join(', ')}<br>`; }
        if (newTags > 0) { message += `• ${newTags} new tag${newTags > 1 ? 's' : ''}: ${this._newTagsCreated.join(', ')}<br>`; }
        message += '<br><em>Please complete their details in the Category & Tags Dashboard after saving this post.</em>';
        alertContainer.innerHTML = `<div class="mdx-alert-box mdx-alert-info" style="margin: 14px;">${this._icon('alert')}<div>${message}</div></div>`;
    }

    _onCategoriesList(data) { this._allCategories = data.categories || []; }
    _onTagsList(data) { this._allTags = data.tags || []; }
    _onCategoryCreated(data) { if (data.success && data.category) { this._allCategories.push(data.category); } }
    _onTagCreated(data) { if (data.success && data.tag) { this._allTags.push(data.tag); } }
    ...
    _wireRelatedPosts() {
        ...
        if (searchInput) {
            let searchTimeout;
            searchInput.addEventListener('input', (e) => { clearTimeout(searchTimeout); searchTimeout = setTimeout(() => { this._renderRelatedPostsList(e.target.value); }, 300); });
        }
        if (clearBtn) { clearBtn.addEventListener('click', () => { if (searchInput) searchInput.value = ''; this._renderRelatedPostsList(''); }); }
    }
    ...
    _wireSchema() {
        this.querySelectorAll('.mdx-schema-type-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                btn.classList.add('active'); this._schemaType = btn.dataset.type;
                this._toggleSchemaFields(); this._updateSchemaPreview();
            });
        });
        this.querySelector('#testRichResultsBtn').addEventListener('click', () => { window.open('https://search.google.com/test/rich-results', '_blank'); });
        this.querySelector('#addAuthorBtn').addEventListener('click', () => {
            ...
            this._meta.structuredData.authors.push({ name: '', url: '' }); this._renderSchemaAuthors(); this._updateSchemaPreview();
        });
        this.querySelector('#addFaqBtn').addEventListener('click', () => {
            ...
            this._meta.structuredData.faqItems.push({ question: '', answer: '' }); this._renderSchemaFAQ(); this._updateSchemaPreview();
        });
        this.querySelector('#addIngredientBtn')?.addEventListener('click', () => {
            ...
            this._meta.structuredData.recipe.ingredients.push(''); this._renderRecipeIngredients(); this._updateSchemaPreview();
        });
        this.querySelector('#addInstructionBtn')?.addEventListener('click', () => {
            ...
            this._meta.structuredData.recipe.instructions.push({ name: '', text: '' }); this._renderRecipeInstructions(); this._updateSchemaPreview();
        });
        ['job-title', 'job-description', 'job-datePosted', 'job-validThrough', 'job-employmentType', 'job-jobLocationType',
         ...
         'job-salaryUnit', 'job-applicantLocationRequirements'].forEach(id => {
            ...
            if (el) { el.addEventListener('input', () => { const key = id.replace('job-', ''); this._meta.structuredData.jobPosting[key] = el.value; this._updateSchemaPreview(); }); }
        });
        ['img-contentUrl', 'img-license', 'img-acquireLicensePage', 'img-creditText', 'img-creatorName', 'img-copyrightNotice'].forEach(id => {
            const el = this.querySelector(`#${id}`);
            if (el) { el.addEventListener('input', () => { const key = id.replace('img-', ''); this._meta.structuredData.imageObject[key] = el.value; this._updateSchemaPreview(); }); }
        });
        ['recipe-name', 'recipe-description', 'recipe-cuisine', 'recipe-category', 'recipe-keywords',
         'recipe-prepTime', 'recipe-cookTime', 'recipe-totalTime', 'recipe-recipeYield', 'recipe-calories'].forEach(id => {
            const el = this.querySelector(`#${id}`);
            if (el) { el.addEventListener('input', () => { const key = id.replace('recipe-', ''); this._meta.structuredData.recipe[key] = el.value; this._updateSchemaPreview(); }); }
        });
    ...
    _toggleSchemaFields() {
        ...
        articleFields.style.display = 'none'; authorsSection.style.display = 'none'; faqSection.style.display = 'none';
        jobFields.style.display = 'none'; imageFields.style.display = 'none'; recipeFields.style.display = 'none'; bestPractices.innerHTML = '';
        if (['Article', 'NewsArticle', 'BlogPosting'].includes(this._schemaType)) {
            articleFields.style.display = 'block'; authorsSection.style.display = 'block'; faqSection.style.display = 'block';
            bestPractices.innerHTML = `<div class="mdx-alert-box mdx-alert-warning">${this._icon('alert')}<div><strong>Best Practices:</strong><br>• Ensure headline is under 110 characters<br>• Add high-quality images (1200x675px recommended)<br>• FAQ items must match content visible on the page<br>• Use real author names with profile URLs</div></div>`;
        } else if (this._schemaType === 'JobPosting') {
            jobFields.style.display = 'block';
            bestPractices.innerHTML = `<div class="mdx-alert-box mdx-alert-warning">${this._icon('alert')}<div><strong>Important:</strong><br>• Set "Valid Through" date accurately<br>• Remove or expire job postings when no longer open<br>• For remote jobs, use TELECOMMUTE job location type<br>• Provide complete salary information when possible</div></div>`;
        } else if (this._schemaType === 'ImageObject') {
            imageFields.style.display = 'block';
            bestPractices.innerHTML = `<div class="mdx-alert-box mdx-alert-warning">${this._icon('alert')}<div><strong>Best Practices:</strong><br>• Provide valid license URL<br>• Include creator information for proper attribution<br>• Ensure license page explains usage rights<br>• Use high-resolution images (minimum 1024px)</div></div>`;
        } else if (this._schemaType === 'Recipe') {
            recipeFields.style.display = 'block'; this._renderRecipeIngredients(); this._renderRecipeInstructions();
            bestPractices.innerHTML = `<div class="mdx-alert-box mdx-alert-warning">${this._icon('alert')}<div><strong>Best Practices:</strong><br>• Use ISO 8601 duration format (PT1H30M = 1 hour 30 min)<br>• Include high-quality step-by-step images<br>• Provide accurate nutritional information<br>• List all ingredients with precise measurements</div></div>`;
        }
    }
    ...
    _renderSchemaAuthors() {
        ...
        container.querySelectorAll('input').forEach(input => {
            input.addEventListener('input', (e) => { const idx = parseInt(e.target.dataset.authorIdx); const field = e.target.dataset.field; this._meta.structuredData.authors[idx][field] = e.target.value; this._updateSchemaPreview(); });
        });
        container.querySelectorAll('.mdx-schema-author-remove').forEach(btn => {
            btn.addEventListener('click', (e) => { const idx = parseInt(e.target.dataset.authorIdx); this._meta.structuredData.authors.splice(idx, 1); this._renderSchemaAuthors(); this._updateSchemaPreview(); });
        });
    }
    ...
    _renderSchemaFAQ() {
        ...
        container.innerHTML = items.map((item, idx) => `
            <div class="mdx-schema-faq-item">
                <div class="mdx-schema-faq-header"><strong>FAQ #${idx + 1}</strong><button class="mdx-schema-faq-remove" data-faq-idx="${idx}">Remove</button></div>
                <div class="mdx-mfield mdx-mfull"><label>Question</label><input class="mdx-minp" type="text" placeholder="Enter question" value="${item.question || ''}" data-faq-idx="${idx}" data-field="question"></div>
                <div class="mdx-mfield mdx-mfull"><label>Answer</label><textarea class="mdx-mtxt" placeholder="Enter answer" data-faq-idx="${idx}" data-field="answer" rows="3">${item.answer || ''}</textarea></div>
            </div>
        `).join('');
        container.querySelectorAll('input, textarea').forEach(input => {
            input.addEventListener('input', (e) => { const idx = parseInt(e.target.dataset.faqIdx); const field = e.target.dataset.field; this._meta.structuredData.faqItems[idx][field] = e.target.value; this._updateSchemaPreview(); });
        });
        container.querySelectorAll('.mdx-schema-faq-remove').forEach(btn => {
            btn.addEventListener('click', (e) => { const idx = parseInt(e.target.dataset.faqIdx); this._meta.structuredData.faqItems.splice(idx, 1); this._renderSchemaFAQ(); this._updateSchemaPreview(); });
        });
    }
    ...
    _renderRecipeIngredients() {
        ...
        container.innerHTML = ingredients.map((ing, idx) => `
            <div class="mdx-schema-list-item">
                <div class="mdx-schema-list-header"><strong>Ingredient #${idx + 1}</strong><button class="mdx-schema-faq-remove" data-ing-idx="${idx}">Remove</button></div>
                <input class="mdx-minp" type="text" placeholder="400ml of pineapple juice" value="${ing}" data-ing-idx="${idx}" style="margin-top:8px;">
            </div>
        `).join('');
        container.querySelectorAll('input').forEach(input => {
            input.addEventListener('input', (e) => { const idx = parseInt(e.target.dataset.ingIdx); this._meta.structuredData.recipe.ingredients[idx] = e.target.value; this._updateSchemaPreview(); });
        });
        container.querySelectorAll('.mdx-schema-faq-remove').forEach(btn => {
            btn.addEventListener('click', (e) => { const idx = parseInt(e.target.dataset.ingIdx); this._meta.structuredData.recipe.ingredients.splice(idx, 1); this._renderRecipeIngredients(); this._updateSchemaPreview(); });
        });
    }
    ...
    _renderRecipeInstructions() {
        ...
        container.innerHTML = instructions.map((inst, idx) => `
            <div class="mdx-schema-list-item">
                <div class="mdx-schema-list-header"><strong>Step #${idx + 1}</strong><button class="mdx-schema-faq-remove" data-inst-idx="${idx}">Remove</button></div>
                <div class="mdx-mfield mdx-mfull" style="margin-top:8px;"><label>Step Name</label><input class="mdx-minp" type="text" placeholder="Blend" value="${inst.name || ''}" data-inst-idx="${idx}" data-field="name"></div>
                <div class="mdx-mfield mdx-mfull"><label>Instructions</label><textarea class="mdx-mtxt" placeholder="Blend ingredients until smooth..." data-inst-idx="${idx}" data-field="text" rows="2">${inst.text || ''}</textarea></div>
            </div>
        `).join('');
        container.querySelectorAll('input, textarea').forEach(input => {
            input.addEventListener('input', (e) => { const idx = parseInt(e.target.dataset.instIdx); const field = e.target.dataset.field; this._meta.structuredData.recipe.instructions[idx][field] = e.target.value; this._updateSchemaPreview(); });
        });
        container.querySelectorAll('.mdx-schema-faq-remove').forEach(btn => {
            btn.addEventListener('click', (e) => { const idx = parseInt(e.target.dataset.instIdx); this._meta.structuredData.recipe.instructions.splice(idx, 1); this._renderRecipeInstructions(); this._updateSchemaPreview(); });
        });
    }
    ...
    _generateStructuredData() {
        ...
        if (this._schemaType === 'JobPosting') {
            ...
            const schema = { "@context": "https://schema.org/", "@type": "JobPosting", "title": job.title || this._meta.blogTitle || '', "description": job.description || this._meta.excerpt || '', "identifier": { "@type": "PropertyValue", "name": job.organizationName || '', "value": "JOB_ID_HERE" }, "datePosted": job.datePosted || new Date().toISOString().split('T')[0], "employmentType": job.employmentType || 'FULL_TIME', "hiringOrganization": { "@type": "Organization", "name": job.organizationName || '', "sameAs": job.organizationUrl || '', "logo": job.organizationLogo || '' } };
            if (job.validThrough) { schema.validThrough = new Date(job.validThrough).toISOString(); }
            if (job.jobLocationType === 'TELECOMMUTE') { schema.jobLocationType = "TELECOMMUTE"; if (job.applicantLocationRequirements) { schema.applicantLocationRequirements = { "@type": "Country", "name": job.applicantLocationRequirements }; } }
            else if (job.streetAddress || job.addressLocality) { schema.jobLocation = { "@type": "Place", "address": { "@type": "PostalAddress", "streetAddress": job.streetAddress || '', "addressLocality": job.addressLocality || '', "addressRegion": job.addressRegion || '', "postalCode": job.postalCode || '', "addressCountry": job.addressCountry || 'US' } }; }
            if (job.salaryValue) { schema.baseSalary = { "@type": "MonetaryAmount", "currency": job.salaryCurrency || 'USD', "value": { "@type": "QuantitativeValue", "value": parseFloat(job.salaryValue), "unitText": job.salaryUnit || 'HOUR' } }; }
            return schema;
        }
        if (this._schemaType === 'ImageObject') {
            const img = this._meta.structuredData.imageObject;
            return { "@context": "https://schema.org/", "@type": "ImageObject", "contentUrl": img.contentUrl || this._meta.featuredImage || '', "license": img.license || '', "acquireLicensePage": img.acquireLicensePage || '', "creditText": img.creditText || '', "creator": { "@type": "Person", "name": img.creatorName || this._meta.author || '' }, "copyrightNotice": img.copyrightNotice || '' };
        }
        if (this._schemaType === 'Recipe') {
            const recipe = this._meta.structuredData.recipe;
            const images = []; if (this._meta.featuredImage) images.push(this._meta.featuredImage);
            const schema = { "@context": "https://schema.org/", "@type": "Recipe", "name": recipe.name || this._meta.blogTitle || '', "description": recipe.description || this._meta.excerpt || '', "image": images, "author": { "@type": "Person", "name": this._meta.author || '' }, "datePublished": this._meta.publishedDate ? new Date(this._meta.publishedDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0], "recipeCuisine": recipe.cuisine || '', "recipeCategory": recipe.category || '', "keywords": recipe.keywords || '', "recipeYield": recipe.recipeYield || '', "recipeIngredient": recipe.ingredients || [], "recipeInstructions": (recipe.instructions || []).map((inst, idx) => ({ "@type": "HowToStep", "name": inst.name || `Step ${idx + 1}`, "text": inst.text || '' })) };
            ...
            if (recipe.calories) { schema.nutrition = { "@type": "NutritionInformation", "calories": recipe.calories }; }
            return schema;
        }
        const images = []; if (this._meta.featuredImage) images.push(this._meta.featuredImage);
        const authors = (this._meta.structuredData.authors || []).filter(a => a.name).map(author => ({ "@type": "Person", "name": author.name, "url": author.url || baseUrl }));
        if (!authors.length && this._meta.author) { authors.push({ "@type": "Person", "name": this._meta.author, "url": this._meta.authorUrl || baseUrl }); }
        const baseSchema = { "@context": "https://schema.org", "@type": this._schemaType, "headline": this._meta.blogTitle || '', "description": this._meta.excerpt || '', "image": images, "datePublished": this._meta.publishedDate ? new Date(this._meta.publishedDate).toISOString() : new Date().toISOString(), "dateModified": this._meta.modifiedDate ? new Date(this._meta.modifiedDate).toISOString() : new Date().toISOString(), "author": authors };
        const faqItems = (this._meta.structuredData.faqItems || []).filter(item => item.question && item.answer);
        if (faqItems.length > 0) { return [baseSchema, { "@context": "https://schema.org", "@type": "FAQPage", "mainEntity": faqItems.map(item => ({ "@type": "Question", "name": item.question, "acceptedAnswer": { "@type": "Answer", "text": item.answer } })) }]; }
        return baseSchema;
    }
    ...
    _renderRelatedPostsList(searchQuery = '') {
        ...
        if (searchQuery.trim()) { const query = searchQuery.toLowerCase(); filteredPosts = filteredPosts.filter(p => (p.blogTitle || p.title || '').toLowerCase().includes(query) || (p.slug || '').toLowerCase().includes(query)); }
        if (countEl) { countEl.textContent = `${selectedIds.length} post${selectedIds.length !== 1 ? 's' : ''} selected`; }
        if (!filteredPosts.length) { listEl.innerHTML = '<div class="mdx-state-box"><p>No posts found</p></div>'; return; }
        listEl.innerHTML = filteredPosts.map(post => {
            const isSelected = selectedIds.includes(post._id);
            return `<div class="mdx-related-post ${isSelected ? 'selected' : ''}" data-id="${post._id}"><div class="mdx-related-check">${isSelected ? '✓' : ''}</div><div class="mdx-related-info"><div class="mdx-related-title">${post.blogTitle || post.title || '(Untitled)'}</div><div class="mdx-related-slug">${post.slug || ''}</div></div></div>`;
        }).join('');
        listEl.querySelectorAll('.mdx-related-post').forEach(el => {
            el.addEventListener('click', () => {
                const postId = el.dataset.id; if (!this._meta.relatedPosts) this._meta.relatedPosts = [];
                const index = this._meta.relatedPosts.indexOf(postId);
                if (index > -1) { this._meta.relatedPosts.splice(index, 1); } else { this._meta.relatedPosts.push(postId); }
    ...
    _wireImgZone(zoneId, fileId, prevId, metaKey, progressId, progressFillId, progressLabelId) {
        ...
        file.addEventListener('change', async e => {
            ...
            if (labelEl) labelEl.textContent = 'Optimizing image to WebP...';
    ...
    async _loadToastEditor() {
        if (window.toastui && window.toastui.Editor) { this._toastEditorLoaded = true; return; }
        try {
    ...
    _initToastEditor(initialMarkdown = '') {
        if (!window.toastui || !window.toastui.Editor) { setTimeout(() => this._initToastEditor(initialMarkdown), 500); return; }
        ...
        try {
            this._toastEditor = new toastui.Editor({
                ...
                toolbarItems: [
                    ...
                    ['code', 'codeblock'],
                    [
                        { el: this._createCustomButton('Video', 'video', () => this._insertVideoEmbed()), tooltip: 'Insert YouTube/Vimeo Video' },
                        { el: this._createCustomButton('Embed', 'embed', () => this._showEmbedDialog()), tooltip: 'Insert Rich Embed' },
                        { el: this._createCustomButton('HTML', 'html', () => this._insertHTMLEmbed()), tooltip: 'Insert HTML Embed' },
                        { el: this._createCustomButton('Edit Alt', 'edit', () => this._editImageAlt()), tooltip: 'Edit Image Alt Text' }
                    ]
                ],
                events: {
                    change: () => {
                ...
                hooks: {
                    addImageBlobHook: async (blob, callback) => {
                        try {
                            self._showProgress('Optimizing Image', 'Converting to WebP format...');
                            ...
                            self._setProgressSubtitle('Uploading to server...');
                            
                            self._pendingImageUpload = { callback };
                            self._emit('upload-image', { blockId: 'editor-' + Date.now(), fileData: webpData, filename: webpFilename, optimize: true });
                        } catch (error) {
                            self._hideProgress();
        ...
        } catch (error) { this._toast('error', 'Failed to initialize editor: ' + error.message); }
    }
    ...
    _createCustomButton(text, icon, onClick) {
        ...
        button.addEventListener('click', (e) => { e.preventDefault(); onClick(); });
        return button;
    }
    ...
    _editImageAlt() {
        ...
        const imageRegex = /!\[([^\]]*)\]\(([^)]+)\)/g;
        let match; let foundImage = null;
        while ((match = imageRegex.exec(markdown)) !== null) { foundImage = { fullMatch: match[0], alt: match[1], url: match[2], index: match.index }; }
        if (!foundImage) { this._toast('info', 'Place cursor near an image or select image markdown to edit alt text'); return; }
        const newAlt = prompt('Enter new alt text for the image:', foundImage.alt);
        if (newAlt !== null) { const newImageMd = `![${newAlt}](${foundImage.url})`; const newMarkdown = markdown.replace(foundImage.fullMatch, newImageMd); this._toastEditor.setMarkdown(newMarkdown); this._toast('success', 'Image alt text updated!'); }
    }
    ...
    _insertVideoEmbed() {
        ...
        if (ytMatch) { embedCode = `[youtube:${ytMatch[1]}]`; }
        const vimeoMatch = url.match(/vimeo\.com\/(\d+)/);
        if (vimeoMatch) { embedCode = `[vimeo:${vimeoMatch[1]}]`; }
        if (embedCode) { this._toastEditor.insertText(embedCode); this._toast('success', 'Video embed added!'); }
        else { this._toast('error', 'Invalid video URL'); }
    }
    ...
    _showEmbedDialog() {
        const existing = this.querySelector('.mdx-embed-dialog-overlay');
        if (existing) existing.remove();

        const overlay = document.createElement('div');
        overlay.className = 'mdx-embed-dialog-overlay';
        overlay.innerHTML = `
            <div class="mdx-embed-dialog">
                <h3>${this._icon('embed')} Insert Rich Embed</h3>
                <label>Embed Type</label>
                <select id="embedTypeSelect">
                    <option value="spotify">Spotify (Track/Album/Playlist/Episode)</option>
                    <option value="video-url">Video (Direct URL - MP4/WebM)</option>
                    <option value="youtube">YouTube</option>
                    <option value="vimeo">Vimeo</option>
                    <option value="codepen">CodePen</option>
                    <option value="twitter">Twitter/X Post</option>
                    <option value="gist">GitHub Gist</option>
                    <option value="iframe">Custom iFrame URL</option>
                </select>
                <div id="embedHint" class="mdx-embed-hint"></div>
                <label>URL</label>
                <input type="text" id="embedUrlInput" placeholder="Paste the URL here...">
                <div class="mdx-embed-dialog-actions">
                    <button class="mdx-btn mdx-btn-light" id="embedCancelBtn">Cancel</button>
                    <button class="mdx-btn mdx-btn-accent" id="embedInsertBtn">${this._icon('check')} Insert</button>
                </div>
            </div>
        `;
        this.appendChild(overlay);

        const typeSelect = overlay.querySelector('#embedTypeSelect');
        const hintEl = overlay.querySelector('#embedHint');
        const urlInput = overlay.querySelector('#embedUrlInput');

        const hints = {
            'spotify': 'Paste a Spotify link like: https://open.spotify.com/track/... or /album/... or /playlist/... or /episode/...',
            'video-url': 'Paste a direct video file URL (MP4, WebM, OGG). Example: https://example.com/video.mp4',
            'youtube': 'Paste a YouTube URL: https://youtube.com/watch?v=... or https://youtu.be/...',
            'vimeo': 'Paste a Vimeo URL: https://vimeo.com/123456789',
            'codepen': 'Paste a CodePen URL: https://codepen.io/username/pen/penId',
            'twitter': 'Paste a Twitter/X post URL: https://twitter.com/user/status/...',
            'gist': 'Paste a GitHub Gist URL: https://gist.github.com/user/gistId',
            'iframe': 'Paste any URL to embed as an iframe'
        };

        hintEl.textContent = hints['spotify'];
        typeSelect.addEventListener('change', () => { hintEl.textContent = hints[typeSelect.value] || ''; });

        overlay.querySelector('#embedCancelBtn').addEventListener('click', () => overlay.remove());
        overlay.addEventListener('click', (e) => { if (e.target === overlay) overlay.remove(); });

        overlay.querySelector('#embedInsertBtn').addEventListener('click', () => {
            const type = typeSelect.value;
            const url = urlInput.value.trim();
            if (!url) { this._toast('error', 'Please enter a URL'); return; }

            let embedCode = '';

            if (type === 'spotify') {
                const spotifyMatch = url.match(/open\.spotify\.com\/(track|album|playlist|episode|show)\/([a-zA-Z0-9]+)/);
                if (spotifyMatch) { embedCode = `[spotify:${spotifyMatch[1]}:${spotifyMatch[2]}]`; }
                else { this._toast('error', 'Invalid Spotify URL'); return; }
            } else if (type === 'video-url') {
                if (url.match(/\.(mp4|webm|ogg)(\?|$)/i) || url.includes('video')) {
                    embedCode = `[video-url:${url}]`;
                } else { embedCode = `[video-url:${url}]`; }
            } else if (type === 'youtube') {
                const ytMatch = url.match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]+)/);
                if (ytMatch) { embedCode = `[youtube:${ytMatch[1]}]`; }
                else { this._toast('error', 'Invalid YouTube URL'); return; }
            } else if (type === 'vimeo') {
                const vimeoMatch = url.match(/vimeo\.com\/(\d+)/);
                if (vimeoMatch) { embedCode = `[vimeo:${vimeoMatch[1]}]`; }
                else { this._toast('error', 'Invalid Vimeo URL'); return; }
            } else if (type === 'codepen') {
                const cpMatch = url.match(/codepen\.io\/([^/]+)\/pen\/([a-zA-Z0-9]+)/);
                if (cpMatch) { embedCode = `[codepen:${cpMatch[1]}:${cpMatch[2]}]`; }
                else { this._toast('error', 'Invalid CodePen URL'); return; }
            } else if (type === 'twitter') {
                const twMatch = url.match(/(?:twitter\.com|x\.com)\/([^/]+)\/status\/(\d+)/);
                if (twMatch) { embedCode = `[twitter:${twMatch[1]}:${twMatch[2]}]`; }
                else { this._toast('error', 'Invalid Twitter/X URL'); return; }
            } else if (type === 'gist') {
                const gistMatch = url.match(/gist\.github\.com\/([^/]+)\/([a-f0-9]+)/);
                if (gistMatch) { embedCode = `[gist:${gistMatch[1]}:${gistMatch[2]}]`; }
                else { this._toast('error', 'Invalid Gist URL'); return; }
            } else if (type === 'iframe') {
                embedCode = `[iframe:${url}]`;
            }

            if (embedCode && this._toastEditor) {
                this._toastEditor.insertText('\n\n' + embedCode + '\n\n');
                this._toast('success', `${type.charAt(0).toUpperCase() + type.slice(1)} embed added!`);
                overlay.remove();
            }
        });
    ...
    _showListView() {
        ...
        this._currentView = 'list';
        if (this._toastEditor) { try { this._toastEditor.destroy(); this._toastEditor = null; } catch(e) {} }
        this._emit('load-post-list', {});
    }
    ...
    _openEditor(post) {
        ...
        let initialMarkdown = '';
        if (post && post.content) { initialMarkdown = post.content; this._populateEditor(post); }
        ...
        setTimeout(() => {
            ...
            if (post) { setTimeout(() => { this._runAnalysis(); }, 500); }
        }, 200);
    }
    ...
    _onPostList(data) {
        ...
        if (!this._posts.length) { content.innerHTML = `<div class="mdx-state-box">${this._icon('image')}<p>No posts yet. Click "New Post" to create your first!</p></div>`; return; }
        content.innerHTML = `<table class="mdx-posts-table"><thead><tr><th>Title / Slug</th><th>Category</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead><tbody id="postsBody"></tbody></table>`;
        const tbody = content.querySelector('#postsBody');
        this._posts.forEach((post, idx) => {
            const tr = document.createElement('tr');
            const dateStr = post.publishedDate ? new Date(post.publishedDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : '—';
            ...
            tr.innerHTML = `<td class="mdx-col-title"><div class="mdx-post-title-txt">${displayTitle}</div><div class="mdx-post-slug">${post.slug || ''}</div></td><td>${post.category || '—'}</td><td><span class="mdx-badge ${badgeClass}">${post.status || 'draft'}</span></td><td style="white-space:nowrap;font-size:13px">${dateStr}</td><td><div class="mdx-row-actions"><button class="mdx-btn mdx-btn-light mdx-btn-sm edit-btn" data-i="${idx}">${this._icon('edit')} Edit</button><button class="mdx-btn mdx-btn-red mdx-btn-sm del-btn" data-i="${idx}">${this._icon('trash')} Delete</button></div></td>`;
            tbody.appendChild(tr);
        });
        tbody.querySelectorAll('.edit-btn').forEach(btn => { btn.addEventListener('click', () => { const idx = parseInt(btn.dataset.i); const post = this._posts[idx]; this._openEditor(post); }); });
    ...
    _onDeleteResult(data) {
        if (data.success) { this._toast('success', 'Post deleted.'); this._emit('load-post-list', {}); }
    ...
    _switchTab(tab) {
        ...
        if (tab === 'schema') { this._toggleSchemaFields(); this._updateSchemaPreview(); }
    }
    ...
    _mdToHtml(md) {
        ...
        md = md.replace(/\[spotify:(track|album|playlist|episode|show):([a-zA-Z0-9]+)\]/g, (match, type, id) => {
            const height = (type === 'track') ? '152' : '352';
            return `<div class="spotify-embed"><iframe src="https://open.spotify.com/embed/${type}/${id}?utm_source=generator&theme=0" height="${height}" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe></div>`;
        });
        md = md.replace(/\[video-url:([^\]]+)\]/g, (match, url) => {
            return `<div class="video-embed"><video controls preload="metadata"><source src="${url}">Your browser does not support the video tag.</video></div>`;
        });
        md = md.replace(/\[codepen:([^:]+):([a-zA-Z0-9]+)\]/g, (match, user, penId) => {
            return `<div class="codepen-embed"><iframe src="https://codepen.io/${user}/embed/${penId}?default-tab=result" height="400" style="width:100%;" scrolling="no" frameborder="no" loading="lazy" allowtransparency="true" allowfullscreen="true"></iframe></div>`;
        });
        md = md.replace(/\[twitter:([^:]+):(\d+)\]/g, (match, user, tweetId) => {
            return `<div class="twitter-embed"><blockquote><p>Loading tweet from @${user}...</p><a href="https://twitter.com/${user}/status/${tweetId}" target="_blank">View on Twitter/X →</a></blockquote></div>`;
        });
        md = md.replace(/\[gist:([^:]+):([a-f0-9]+)\]/g, (match, user, gistId) => {
            return `<div class="generic-embed"><iframe src="https://gist.github.com/${user}/${gistId}.pibb" height="300" style="width:100%;" frameborder="0"></iframe></div>`;
        });
        md = md.replace(/\[iframe:([^\]]+)\]/g, (match, url) => {
            return `<div class="generic-embed"><iframe src="${url}" height="400" style="width:100%;" frameborder="0" loading="lazy" allowfullscreen></iframe></div>`;
        });
        md = md.replace(/\[html\]([\s\S]*?)\[\/html\]/g, (match, html) => { return html; });
        ...
        return md
            ...
            .replace(/^(?!<[h1-6ublptd]|<hr|<pre|<div|<block)(.+)$/gm, '<p>$1</p>');
    }
    ...
    _resetEditorState() {
        ...
        this._newTagsCreated = [];
        this._highlightedIssueType = null;
        this.querySelectorAll('[data-m]').forEach(el => { if (el.type === 'checkbox') el.checked = false; else el.value = ''; });
        ['authorPrev', 'featuredPrev', 'ogPrev'].forEach(id => { const el = this.querySelector(`#${id}`); if (el) { el.src = ''; el.style.display = 'none'; } });
        this._seoScore = 0; this._readabilityScore = 0; this._grammarScore = 0;
        this._updateScoreDisplay();
        this.querySelectorAll('.mdx-schema-type-btn').forEach(b => b.classList.remove('active'));
        this.querySelector('.mdx-schema-type-btn[data-type="Article"]')?.classList.add('active');
        this._renderSchemaAuthors(); this._renderSchemaFAQ();
        const alertContainer = this.querySelector('#newItemsAlert');
    ...
    _populateEditor(data) {
        ...
        if (data.structuredData && typeof data.structuredData === 'string') {
            try { this._meta.structuredData = JSON.parse(data.structuredData); this._schemaType = this._meta.structuredData.type || 'Article'; }
            catch(e) { this._meta.structuredData = this._freshMeta().structuredData; }
        }
        this.querySelectorAll('[data-m]').forEach(el => {
            const k = el.dataset.m;
            if (k in this._meta) {
                if (el.type === 'checkbox') { el.checked = !!this._meta[k]; }
                else if (el.type === 'datetime-local') { el.value = this._formatDateForInput(this._meta[k]); }
                else if (el.type === 'number') { el.value = this._meta[k] || 0; }
                else { el.value = this._meta[k] || ''; }
            }
        });
        this._currentMarkdown = data.content || '';
        if (this._meta.authorImage) { const prev = this.querySelector('#authorPrev'); if (prev) { prev.src = this._meta.authorImage; prev.style.display = 'block'; } }
        if (this._meta.featuredImage) { const prev = this.querySelector('#featuredPrev'); if (prev) { prev.src = this._meta.featuredImage; prev.style.display = 'block'; } }
        if (this._meta.seoOgImage) { const prev = this.querySelector('#ogPrev'); if (prev) { prev.src = this._meta.seoOgImage; prev.style.display = 'block'; } }
        this.querySelectorAll('.mdx-schema-type-btn').forEach(b => { b.classList.toggle('active', b.dataset.type === this._schemaType); });
        if (this._schemaType === 'JobPosting') { const job = this._meta.structuredData.jobPosting; Object.keys(job).forEach(key => { const el = this.querySelector(`#job-${key}`); if (el) el.value = job[key] || ''; }); }
        if (this._schemaType === 'ImageObject') { const img = this._meta.structuredData.imageObject; Object.keys(img).forEach(key => { const el = this.querySelector(`#img-${key}`); if (el) el.value = img[key] || ''; }); }
        if (this._schemaType === 'Recipe') { const recipe = this._meta.structuredData.recipe; Object.keys(recipe).forEach(key => { if (key !== 'ingredients' && key !== 'instructions') { const el = this.querySelector(`#recipe-${key}`); if (el) el.value = recipe[key] || ''; } }); }
        this._renderSchemaAuthors(); this._renderSchemaFAQ(); this._renderRecipeIngredients(); this._renderRecipeInstructions(); this._toggleSchemaFields(); this._updateSchemaPreview();
    }

    _save(status) {
    const md = this._cleanMarkdown(this._currentMarkdown || '');
    const publishedDate = this._meta.publishedDate ? this._parseDateFromInput(this._meta.publishedDate) : null;
    const modifiedDate = this._meta.modifiedDate ? this._parseDateFromInput(this._meta.modifiedDate) : new Date();
    const readTime = this._meta.readTime && this._meta.readTime > 0 ? Number(this._meta.readTime) : Math.max(1, Math.ceil(md.split(/\s+/).length / 200));
    const structuredData = JSON.stringify({ type: this._schemaType, ...this._meta.structuredData, schema: this._generateStructuredData() });
    const newCategories = this._newCategoriesCreated;
    const newTags = this._newTagsCreated;
    
    // Use the dropdown value if we're updating, otherwise use button's status
    const finalStatus = this._editPost ? (this._meta.status || status) : status;
    
    const action = finalStatus === 'published' ? (this._editPost ? 'Updating' : 'Publishing') : 'Saving draft';
    this._showProgress(`${action}...`, 'Preparing post data...');
    ...
    this._emit('save-post', {
        ...this._meta, content: md, status: finalStatus, publishedDate, modifiedDate, readTime, structuredData,
        _id: this._editPost?._id || null, newCategories, newTags
    });
}
    ...
    _onSaveResult(data) {
        if (data.success) {
            ...
            this._newCategoriesCreated = []; this._newTagsCreated = []; this._showNewItemsAlert();
        } else {
            this._hideProgress();
    ...
    _onUploadResult(data) {
        if (data.blockId && this._pendingImageUpload) {
            ...
            this._setProgress(100);
            this._setProgressSubtitle('Image uploaded!');
            setTimeout(() => { this._hideProgress(); this._toast('success', 'Image uploaded!'); }, 500);
        }
        if (data.metaKey) {
            ...
            if (prev) { prev.src = this._meta[data.metaKey]; prev.style.display = 'block'; }
            ...
            if (this._pendingMetaProgress) {
                ...
                setTimeout(() => { if (progressEl) progressEl.classList.remove('active'); this._pendingMetaProgress = null; }, 1500);
            }
    ...
    _autoSlug(title) {
        ...
        const existingSlugs = this._posts.filter(p => p._id !== this._editPost?._id).map(p => p.slug);
    ...
    _detectPassiveSentences(sentences) {
        const passivePatterns = [
            /\b(was|were|is|are|am|been|being|be)\s+([\w]+ly\s+)?([\w]+ed|[\w]+en|built|bought|caught|chosen|come|cut|done|drawn|driven|eaten|fallen|felt|found|forgotten|given|gone|grown|had|heard|held|hidden|hit|hung|hurt|kept|known|laid|led|left|let|lain|lost|made|meant|met|paid|put|read|ridden|risen|run|said|seen|sent|set|shown|shut|slept|spoken|spent|stood|stolen|struck|sworn|swept|taken|taught|thought|thrown|told|torn|understood|woken|worn|won|written|brought|dealt|fed|fought|got|gotten|ground|knelt|leapt|learnt|meant|proved|proven|shaken|shone|shot|shrunk|slung|slung|spun|sprung|stung|stunk|strung|stuck|sung|sunk|swept|swum|swung|wept|wound|wrung)\b/i,
            /\b(got|get|gets|getting)\s+([\w]+ed|[\w]+en)\b/i,
        ];
        const results = [];
        sentences.forEach((sentence, idx) => {
            ...
            for (const pattern of passivePatterns) {
                if (pattern.test(trimmed)) {
                    results.push({ index: idx, sentence: trimmed, type: 'passive' });
                    break;
                }
            }
        });
    ...
    _detectLongSentences(sentences) {
        const results = [];
        sentences.forEach((sentence, idx) => {
            const wordCount = sentence.trim().split(/\s+/).filter(w => w.length > 0).length;
            if (wordCount > 25) {
                results.push({ index: idx, sentence: sentence.trim(), wordCount, type: 'long' });
            }
        });
        return results;
    }

    _detectComplexSentences(sentences) {
        const complexWords = /\b(notwithstanding|nevertheless|furthermore|consequently|approximately|accordingly|predominantly|simultaneously|aforementioned|henceforth|hereinafter|inasmuch|insofar|notwithstanding|thereunto|whereby|wherein|whereupon|utilization|implementation|conceptualization|characterization|individualization|operationalization|contextualization|institutionalization|systematization|parameterization)\b/gi;
        const results = [];
        sentences.forEach((sentence, idx) => {
            const trimmed = sentence.trim();
            const matches = trimmed.match(complexWords);
            if (matches && matches.length >= 1) {
                results.push({ index: idx, sentence: trimmed, complexWords: matches, type: 'complex' });
            }
        });
        return results;
    }

    _detectWeakAdverbs(text) {
        const weakAdverbPattern = /\b(very|really|quite|extremely|absolutely|totally|completely|basically|actually|literally|definitely|certainly|obviously|simply|just|rather|somewhat|fairly|pretty|highly)\s+(\w+)/gi;
        const results = [];
        let match;
        while ((match = weakAdverbPattern.exec(text)) !== null) {
            results.push({ phrase: match[0], adverb: match[1], position: match.index });
        }
        return results;
    }

    _highlightIssuesInEditor(issueType) {
        if (!this._toastEditor) return;
        
        if (this._highlightedIssueType === issueType) {
        ...
        this._clearHighlights();
        this._highlightedIssueType = issueType;
        
        const editorEl = this.querySelector('.toastui-editor-ww-container');
        if (!editorEl) return;
        
        const contentEl = editorEl.querySelector('.ProseMirror') || editorEl.querySelector('.toastui-editor-contents');
        if (!contentEl) return;
        
        const paragraphs = contentEl.querySelectorAll('p, li, td, th, blockquote');
        const textContent = this._getPlainText();
        const sentences = textContent.split(/(?<=[.!?])\s+/).filter(s => s.trim().length > 0);
        
        let issueSentences = [];
        
        if (issueType === 'passive') {
            issueSentences = this._detectPassiveSentences(sentences).map(r => r.sentence);
        } else if (issueType === 'long') {
            issueSentences = this._detectLongSentences(sentences).map(r => r.sentence);
        } else if (issueType === 'complex') {
            issueSentences = this._detectComplexSentences(sentences).map(r => r.sentence);
        } else if (issueType === 'adverb') {
            const adverbs = this._detectWeakAdverbs(textContent);
            this._highlightPhrasesInContent(contentEl, adverbs.map(a => a.phrase), 'mdx-hl-adverb');
            this._updateHighlightButtons();
            return;
        } else if (issueType === 'repeated') {
            const words = textContent.split(/\s+/).map(w => w.toLowerCase().replace(/[^a-z]/g, '')).filter(w => w.length > 5);
            const freq = {};
            words.forEach(w => { freq[w] = (freq[w] || 0) + 1; });
            const repeated = Object.entries(freq).filter(([w, c]) => c > 5).map(([w]) => w);
            this._highlightPhrasesInContent(contentEl, repeated, 'mdx-hl-repeated');
            this._updateHighlightButtons();
            return;
        }
        
        if (issueSentences.length > 0) {
            const hlClass = issueType === 'passive' ? 'mdx-hl-passive' : issueType === 'long' ? 'mdx-hl-longsentence' : 'mdx-hl-complex';
            this._highlightSentencesInContent(contentEl, issueSentences, hlClass);
        }
        
        this._updateHighlightButtons();
    }

    _highlightSentencesInContent(contentEl, sentences, hlClass) {
        ...
        for (const sentence of sentences) {
            ...
            if (shortSentence.length < 10) continue;
            
            for (const node of textNodes) {
                const nodeText = node.textContent;
                const idx = nodeText.indexOf(shortSentence);
                if (idx !== -1) {
                    try {
                        const range = document.createRange();
                        const sentenceEnd = nodeText.indexOf('.', idx + shortSentence.length);
                        const endIdx = sentenceEnd !== -1 ? Math.min(sentenceEnd + 1, nodeText.length) : Math.min(idx + sentence.length, nodeText.length);
    ...
    _highlightPhrasesInContent(contentEl, phrases, hlClass) {
        const walker = document.createTreeWalker(contentEl, NodeFilter.SHOW_TEXT, null, false);
        const textNodes = [];
        while (walker.nextNode()) { textNodes.push(walker.currentNode); }
        ...
        for (const phrase of phrases) {
            ...
            for (let i = 0; i < textNodes.length; i++) {
                ...
                const idx = nodeText.indexOf(lowerPhrase);
                if (idx !== -1) {
                    try {
                        const range = document.createRange();
                        range.setStart(node, idx);
    ...
    _clearHighlights() {
        ...
        const hlClasses = ['mdx-hl-passive', 'mdx-hl-longsentence', 'mdx-hl-complex', 'mdx-hl-adverb', 'mdx-hl-spelling', 'mdx-hl-repeated'];
        hlClasses.forEach(cls => {
            contentEl.querySelectorAll(`.${cls}`).forEach(span => {
    ...
    _updateHighlightButtons() {
        this.querySelectorAll('.mdx-highlight-btn').forEach(btn => {
            btn.classList.toggle('active', btn.dataset.hlType === this._highlightedIssueType);
        });
    }
    ...
    _runGrammarAnalysis() {
        ...
        const passiveResults = this._detectPassiveSentences(sentences);
        const passiveCount = passiveResults.length;
        const passiveRatio = passiveCount / Math.max(sentences.length, 1);
        
        if (passiveRatio > 0.3) {
            score -= 20;
            checks.push({ status: 'bad', text: `Too much passive voice (${passiveCount} of ${sentences.length} sentences). Use active voice.`, hlType: 'passive' });
        } else if (passiveRatio > 0.15) {
            score -= 10;
            checks.push({ status: 'ok', text: `Some passive voice detected (${passiveCount} sentences). Consider active alternatives.`, hlType: 'passive' });
        } else if (passiveCount > 0) {
            score -= 3;
            checks.push({ status: 'ok', text: `Minimal passive voice (${passiveCount} sentence${passiveCount > 1 ? 's' : ''})`, hlType: 'passive' });
        } else {
            checks.push({ status: 'good', text: 'No passive voice detected', hlType: 'passive' });
        }

        const longResults = this._detectLongSentences(sentences);
        const longCount = longResults.length;
        const longRatio = longCount / Math.max(sentences.length, 1);
        
        if (longRatio > 0.4) {
            score -= 18;
            checks.push({ status: 'bad', text: `${longCount} very long sentences (>25 words). Break them up for clarity.`, hlType: 'long' });
        } else if (longCount > 0) {
            score -= longCount * 3;
            checks.push({ status: 'ok', text: `${longCount} long sentence${longCount > 1 ? 's' : ''} detected (>25 words)`, hlType: 'long' });
        } else {
            checks.push({ status: 'good', text: 'All sentences are concise', hlType: 'long' });
        }

        const complexResults = this._detectComplexSentences(sentences);
        if (complexResults.length > 0) {
            score -= complexResults.length * 4;
            const allComplex = [...new Set(complexResults.flatMap(r => r.complexWords))].slice(0, 5);
            checks.push({ status: 'ok', text: `Complex vocabulary: ${allComplex.map(w => `"${w}"`).join(', ')}. Consider simpler alternatives.`, hlType: 'complex' });
        } else {
            checks.push({ status: 'good', text: 'Good vocabulary accessibility', hlType: 'complex' });
        }

        const weakAdverbs = this._detectWeakAdverbs(textContent);
        if (weakAdverbs.length > 5) {
            score -= 12;
            const examples = weakAdverbs.slice(0, 3).map(a => `"${a.phrase}"`).join(', ');
            checks.push({ status: 'bad', text: `${weakAdverbs.length} weak adverb phrases found: ${examples}. Use stronger words.`, hlType: 'adverb' });
        } else if (weakAdverbs.length > 2) {
            score -= 6;
            checks.push({ status: 'ok', text: `${weakAdverbs.length} weak adverb combinations detected`, hlType: 'adverb' });
        } else {
            checks.push({ status: 'good', text: 'Strong word choices throughout', hlType: 'adverb' });
        }

        const wordFreq = {};
        words.forEach(w => { const lw = w.toLowerCase().replace(/[^a-z]/g, ''); if (lw.length > 5) { wordFreq[lw] = (wordFreq[lw] || 0) + 1; } });
        const overusedWords = Object.entries(wordFreq).filter(([w, c]) => c > 5).sort((a, b) => b[1] - a[1]).slice(0, 5);
        if (overusedWords.length > 0) {
            score -= overusedWords.length * 3;
            const wordList = overusedWords.map(([w, c]) => `"${w}" (${c}×)`).join(', ');
            checks.push({ status: 'ok', text: `Repetitive words: ${wordList}`, hlType: 'repeated' });
        } else {
            checks.push({ status: 'good', text: 'Good word variety', hlType: 'repeated' });
        }

        if (this._typoChecker) {
            const uniqueWords = [...new Set(words.map(w => w.replace(/[^a-zA-Z]/g, '')).filter(w => w.length > 2))];
            let spellingErrors = 0;
            const misspelled = [];
            for (const word of uniqueWords) {
                if (word.match(/^[A-Z]{2,}$/) || word.match(/^[A-Z][a-z]+[A-Z]/)) continue;
                try {
                    if (!this._typoChecker.check(word)) {
                        spellingErrors++;
                        if (misspelled.length < 5) {
                            const suggestions = this._typoChecker.suggest(word).slice(0, 2);
                            misspelled.push(suggestions.length ? `"${word}" → ${suggestions.join('/')}` : `"${word}"`);
                        }
                    }
                } catch(e) {}
            }
            if (spellingErrors > 5) {
                score -= 15;
                checks.push({ status: 'bad', text: `${spellingErrors} potential spelling errors: ${misspelled.join(', ')}` });
            } else if (spellingErrors > 0) {
                score -= spellingErrors * 2;
                checks.push({ status: 'ok', text: `${spellingErrors} possible misspelling${spellingErrors > 1 ? 's' : ''}: ${misspelled.join(', ')}` });
            } else {
                checks.push({ status: 'good', text: 'No spelling errors detected' });
            }
        }
        ...
        sentences.forEach(s => {
            ...
            if (firstWord) { sentenceStarters[firstWord] = (sentenceStarters[firstWord] || 0) + 1; }
        });
        const repetitiveStarters = Object.entries(sentenceStarters).filter(([w, c]) => c > 3 && sentences.length > 5);
        if (repetitiveStarters.length > 0) {
            score -= repetitiveStarters.length * 3;
            const starterList = repetitiveStarters.map(([w, c]) => `"${w}" (${c}×)`).join(', ');
            checks.push({ status: 'ok', text: `Repetitive sentence starters: ${starterList}. Vary your openings.` });
        } else if (sentences.length > 3) {
            checks.push({ status: 'good', text: 'Good variety in sentence openings' });
        }
    ...
    _runSEOAnalysis() {
        ...
        if (keyphrase.length > 0) { score += 8; checks.push({ status: 'good', text: 'Focus keyphrase is set' }); }
        else { checks.push({ status: 'bad', text: 'Set a focus keyphrase to target' }); }
        ...
        if (keyphrase && blogTitle.toLowerCase().includes(keyphrase.toLowerCase())) {
            ...
            if (position === 0) { score += 12; checks.push({ status: 'good', text: 'Keyphrase appears at the beginning of title' }); }
            else { score += 8; checks.push({ status: 'ok', text: 'Keyphrase appears in title but not at the beginning' }); }
        } else if (keyphrase) { checks.push({ status: 'bad', text: 'Keyphrase should appear in blog title, preferably at the beginning' }); }

        if (keyphrase && seoTitle.toLowerCase().includes(keyphrase.toLowerCase())) { score += 10; checks.push({ status: 'good', text: 'Keyphrase appears in SEO title' }); }
        else if (keyphrase && seoTitle) { checks.push({ status: 'bad', text: 'Add keyphrase to SEO title' }); }

        if (seoTitle.length >= 50 && seoTitle.length <= 60) { score += 10; checks.push({ status: 'good', text: `SEO title length is optimal (${seoTitle.length} chars)` }); }
        else if (seoTitle.length > 0 && seoTitle.length < 70) { score += 5; checks.push({ status: 'ok', text: `SEO title: ${seoTitle.length} chars (optimal: 50-60)` }); }
        else if (seoTitle.length > 70) { checks.push({ status: 'bad', text: `SEO title too long (${seoTitle.length} chars, max 60)` }); }
        else { checks.push({ status: 'bad', text: 'Add SEO title (50-60 characters)' }); }

        if (seoDesc.length >= 120 && seoDesc.length <= 160) { score += 12; checks.push({ status: 'good', text: `Meta description length optimal (${seoDesc.length} chars)` }); }
        else if (seoDesc.length >= 100 && seoDesc.length < 170) { score += 7; checks.push({ status: 'ok', text: `Meta description: ${seoDesc.length} chars (optimal: 120-160)` }); }
        else if (seoDesc.length > 170) { checks.push({ status: 'bad', text: `Meta description too long (${seoDesc.length} chars, max 160)` }); }
        else { checks.push({ status: 'bad', text: 'Add meta description (120-160 characters)' }); }

        if (keyphrase && seoDesc.toLowerCase().includes(keyphrase.toLowerCase())) { score += 10; checks.push({ status: 'good', text: 'Keyphrase appears in meta description' }); }
        else if (keyphrase && seoDesc) { checks.push({ status: 'bad', text: 'Include keyphrase in meta description' }); }

        if (wordCount >= 600) { score += 12; checks.push({ status: 'good', text: `Excellent content length (${wordCount} words)` }); }
        else if (wordCount >= 300) { score += 8; checks.push({ status: 'ok', text: `Good content length (${wordCount} words)` }); }
        else if (wordCount >= 150) { score += 4; checks.push({ status: 'ok', text: `Content is short (${wordCount} words, aim for 300+)` }); }
        else { checks.push({ status: 'bad', text: `Content too short (${wordCount} words, minimum 300)` }); }

        if (keyphrase && content) {
            const keyphraseCount = (content.toLowerCase().match(new RegExp(keyphrase.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []).length;
            const density = wordCount > 0 ? (keyphraseCount / wordCount) * 100 : 0;
            if (density >= 0.5 && density <= 2.5) { score += 8; checks.push({ status: 'good', text: `Keyphrase density good (${density.toFixed(1)}%)` }); }
            else if (density > 0 && density < 0.5) { score += 4; checks.push({ status: 'ok', text: `Use keyphrase more (${density.toFixed(1)}%, aim 0.5-2.5%)` }); }
            else if (density > 2.5) { checks.push({ status: 'bad', text: `Keyphrase overused (${density.toFixed(1)}%, max 2.5%)` }); }
            else { checks.push({ status: 'bad', text: 'Keyphrase not found in content' }); }
            const firstParagraph = content.split('\n\n')[0] || '';
            if (firstParagraph.toLowerCase().includes(keyphrase.toLowerCase())) { score += 8; checks.push({ status: 'good', text: 'Keyphrase in first paragraph' }); }
            else if (wordCount > 50) { checks.push({ status: 'bad', text: 'Add keyphrase to first paragraph' }); }
        }

        const h2Count = (content.match(/^## /gm) || []).length;
        if (h2Count > 0) { score += 6; checks.push({ status: 'good', text: `${h2Count} subheading${h2Count > 1 ? 's' : ''} found` }); }
        else if (wordCount > 300) { checks.push({ status: 'bad', text: 'Add subheadings (H2) to structure content' }); }

        const images = (content.match(/!\[/g) || []).length;
        if (images > 0) {
            score += 4; checks.push({ status: 'good', text: `${images} image${images > 1 ? 's' : ''} in content` });
            const altMissing = (content.match(/!\[\]\(/g) || []).length;
            if (altMissing > 0) { checks.push({ status: 'bad', text: `${altMissing} image${altMissing > 1 ? 's' : ''} missing alt text` }); }
            else { score += 4; checks.push({ status: 'good', text: 'All images have alt text' }); }
        } else if (wordCount > 300) { checks.push({ status: 'ok', text: 'Consider adding images' }); }

        const internalLinks = (content.match(/\[([^\]]+)\]\((?!http)/g) || []).length;
        if (internalLinks >= 2) { score += 6; checks.push({ status: 'good', text: `${internalLinks} internal link${internalLinks > 1 ? 's' : ''}` }); }
        else if (internalLinks === 1) { score += 3; checks.push({ status: 'ok', text: 'Add more internal links (2+ recommended)' }); }
        else if (wordCount > 300) { checks.push({ status: 'bad', text: 'Add internal links to other content' }); }

        const externalLinks = (content.match(/\[([^\]]+)\]\(http/g) || []).length;
        if (externalLinks > 0 && externalLinks <= 5) { score += 4; checks.push({ status: 'good', text: `${externalLinks} external link${externalLinks > 1 ? 's' : ''}` }); }
        else if (externalLinks > 5) { checks.push({ status: 'ok', text: 'Many external links - ensure quality sources' }); }
    ...
    _runReadabilityAnalysis() {
        ...
        if (wordCount === 0) { this._readabilityScore = 0; this._readabilityAnalysis = [{ status: 'bad', text: 'Start writing to analyze readability' }]; this._updateScoreDisplay(); return; }

        const avgSentenceLength = wordCount / Math.max(sentences.length, 1);
        if (avgSentenceLength <= 15) { score += 20; checks.push({ status: 'good', text: `Excellent sentence length (avg ${avgSentenceLength.toFixed(1)} words)` }); }
        else if (avgSentenceLength <= 20) { score += 15; checks.push({ status: 'good', text: `Good sentence length (avg ${avgSentenceLength.toFixed(1)} words)` }); }
        else if (avgSentenceLength <= 25) { score += 10; checks.push({ status: 'ok', text: `Acceptable sentences (avg ${avgSentenceLength.toFixed(1)} words, aim <20)` }); }
        else { score += 5; checks.push({ status: 'bad', text: `Sentences too long (avg ${avgSentenceLength.toFixed(1)} words, aim <20)` }); }

        const longSentences = sentences.filter(s => s.split(/\s+/).length > 25).length;
        const longSentenceRatio = longSentences / Math.max(sentences.length, 1);
        if (longSentenceRatio === 0) { score += 15; checks.push({ status: 'good', text: 'No overly long sentences', hlType: 'long' }); }
        else if (longSentenceRatio < 0.25) { score += 10; checks.push({ status: 'ok', text: `${longSentences} long sentence${longSentences > 1 ? 's' : ''} (>25 words)`, hlType: 'long' }); }
        else { score += 3; checks.push({ status: 'bad', text: `${longSentences} very long sentences - split them up`, hlType: 'long' }); }
        ...
        if (paragraphRatio === 0 && paragraphs.length > 0) { score += 15; checks.push({ status: 'good', text: 'All paragraphs concise' }); }
        else if (paragraphRatio < 0.3) { score += 10; checks.push({ status: 'ok', text: 'Most paragraphs good length' }); }
        else { score += 3; checks.push({ status: 'bad', text: `${longParagraphs} long paragraph${longParagraphs > 1 ? 's' : ''} (>150 words)` }); }

        const headings = (content.match(/^#{2,6}\s/gm) || []).length;
        const wordsPerHeading = headings > 0 ? wordCount / headings : wordCount;
        if (headings > 0 && wordsPerHeading <= 250) { score += 15; checks.push({ status: 'good', text: 'Excellent use of subheadings' }); }
        else if (headings > 0 && wordsPerHeading <= 400) { score += 10; checks.push({ status: 'ok', text: 'Good subheading distribution' }); }
        else if (headings > 0) { score += 5; checks.push({ status: 'ok', text: 'Add more subheadings (every 250-300 words)' }); }
        else if (wordCount > 300) { checks.push({ status: 'bad', text: 'Add subheadings to break up text' }); }

        const transitionWords = ['however','therefore','furthermore','moreover','nevertheless','consequently','additionally','meanwhile','similarly','likewise','thus','hence','also','besides','first','second','third','finally','for example','for instance','in addition','as a result','on the other hand','in contrast','in conclusion'];
        const transitionCount = transitionWords.filter(word => textContent.toLowerCase().includes(word)).length;
        const transitionRatio = transitionCount / Math.max(paragraphs.length, 1);
        if (transitionRatio >= 0.3) { score += 12; checks.push({ status: 'good', text: 'Excellent use of transition words' }); }
        else if (transitionRatio >= 0.2) { score += 8; checks.push({ status: 'ok', text: 'Good use of transition words' }); }
        else if (transitionCount > 0) { score += 4; checks.push({ status: 'ok', text: 'Use more transition words for better flow' }); }
        else if (wordCount > 200) { checks.push({ status: 'bad', text: 'Add transition words to improve flow' }); }

        const passiveResults = this._detectPassiveSentences(sentences);
        const passiveRatio = passiveResults.length / Math.max(sentences.length, 1);
        if (passiveRatio < 0.1) { score += 15; checks.push({ status: 'good', text: 'Excellent - very little passive voice', hlType: ' passive' }); }
        else if (passiveRatio < 0.2) { score += 12; checks.push({ status: 'good', text: 'Minimal passive voice', hlType: 'passive' }); }
        else if (passiveRatio < 0.4) { score += 6; checks.push({ status: 'ok', text: 'Some passive voice - use more active voice', hlType: 'passive' }); }
        else { checks.push({ status: 'bad', text: 'Too much passive voice - rewrite in active voice', hlType: 'passive' }); }

        const consecutiveSentences = this._findConsecutiveSentences(sentences);
        if (consecutiveSentences === 0) { score += 8; checks.push({ status: 'good', text: 'Good sentence variety' }); }
        else if (consecutiveSentences < 3) { score += 5; checks.push({ status: 'ok', text: 'Vary sentence structure more' }); }
        else { checks.push({ status: 'bad', text: `${consecutiveSentences} consecutive similar sentences` }); }
    ...
    _updateScoreDisplay() {
        ...
        if (grammarCircle && grammarText && grammarLabel) {
            ...
            let color = '#cf1322'; let label = 'Needs work';
            if (this._grammarScore >= 90) { color = '#389e0d'; label = 'Excellent!'; }
            else if (this._grammarScore >= 75) { color = '#52c41a'; label = 'Great!'; }
            else if (this._grammarScore >= 60) { color = '#fa8c16'; label = 'Good'; }
            grammarCircle.style.stroke = color; grammarText.style.color = color; grammarLabel.textContent = label; grammarLabel.style.color = color;
        }

        if (grammarItems) {
            grammarItems.innerHTML = this._grammarAnalysis.map(item => {
                const hlBtn = item.hlType ? `<button class="mdx-highlight-btn${this._highlightedIssueType === item.hlType ? ' active' : ''}" data-hl-type="${item.hlType}" title="Highlight in editor">${this._icon('highlight')}</button>` : '';
                return `<div class="mdx-analysis-item mdx-analysis-${item.status}">
                    <div class="mdx-analysis-icon">${item.status === 'good' ? '✓' : item.status === 'ok' ? '!' : '✕'}</div>
                    <div class="mdx-analysis-text">${item.text}</div>
                    ${hlBtn}
                </div>`;
            }).join('');
            grammarItems.querySelectorAll('.mdx-highlight-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    this._highlightIssuesInEditor(btn.dataset.hlType);
                });
            });
        }
        ...
        if (seoCircle && seoText && seoLabel) {
            ...
            let color = '#cf1322'; let label = 'Needs improvement';
            if (this._seoScore >= 80) { color = '#389e0d'; label = 'Great!'; }
            else if (this._seoScore >= 60) { color = '#fa8c16'; label = 'Good'; }
            else if (this._seoScore >= 40) { color = '#fa8c16'; label = 'OK'; }
            seoCircle.style.stroke = color; seoText.style.color = color; seoLabel.textContent = label; seoLabel.style.color = color;
        }
        ...
        if (readCircle && readText && readLabel) {
            ...
            let color = '#cf1322'; let label = 'Needs improvement';
            if (this._readabilityScore >= 80) { color = '#389e0d'; label = 'Easy to read!'; }
            else if (this._readabilityScore >= 60) { color = '#fa8c16'; label = 'Fairly easy'; }
            else if (this._readabilityScore >= 40) { color = '#fa8c16'; label = 'OK'; }
            readCircle.style.stroke = color; readText.style.color = color; readLabel.textContent = label; readLabel.style.color = color;
        }
        ...
        if (readItems) {
            ...
            readItems.querySelectorAll('.mdx-highlight-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    this._highlightIssuesInEditor(btn.dataset.hlType);
                });
