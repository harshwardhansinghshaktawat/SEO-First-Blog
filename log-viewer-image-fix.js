class EnhancedBlogPostViewer extends HTMLElement {
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    if (name === 'post-data') {
      try {
        ...
        this.renderPost();
        this.updateSEOMarkup(); // Update SEO markup when post data changes
      } catch (e) {
  ...
  // Convert Wix image URL to proper HTTPS URL
  _convertWixImageUrl(wixUrl) {
    if (!wixUrl || typeof wixUrl !== 'string') {
      return 'https://static.wixstatic.com/media/default-image.jpg';
    }

    // If already a proper URL, return it
    if (wixUrl.startsWith('http://') || wixUrl.startsWith('https://')) {
      return wixUrl;
    }

    // Handle Wix media URLs (wix:image://v1/...)
    if (wixUrl.startsWith('wix:image://')) {
      try {
        // Extract the file ID from the Wix URL
        const parts = wixUrl.split('/');
        const fileId = parts[3]?.split('#')[0]; // Get file ID and remove any hash
  ...
  renderPost() {
    ...
    // Convert author image URL
    const authorImageUrl = this._convertWixImageUrl(post.authorImage);

    // Render meta section (removed title and featured image)
  ...
  _renderContent(markdown) {
    ...
    // Convert Wix image URLs in the HTML content
  ...
  // Convert Wix image URLs in HTML content
  ...
  // Generate SEO markup for search engines
  updateSEOMarkup() {
    if (!this.state.postData) return;

    const post = this.state.postData;
    
    // Create plain HTML version for SEO bots
    let seoHTML = '';

    // Add author and meta info
    ...
    // Add main content (convert markdown to HTML)
    if (post.content) {
    ...
    // Dispatch event to set SEO markup
  ...
  loadMarkedJS() {
    ...
    script.async = true;
    script.onload = () => { this.markedLoaded = true; };
