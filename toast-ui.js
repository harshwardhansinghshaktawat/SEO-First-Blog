// ============================================================
// MDX BLOG EDITOR — Custom Element v6 (ALL FIXES)
...
class MdxBlogEditor extends HTMLElement {
    ...
    attributeChangedCallback(name, oldVal, newVal) {
        ...
        // Allow post-list to work even before full initialization
        if (name === 'post-list') {
    ...
    connectedCallback() {
        ...
        requestAnimationFrame(() => {
            ...
            // Process pending post list if any
    ...
    _styles() { 
        // Keep the same styles from before - no changes needed
        return `
...
<div class="mdx-msec">
    ...
    // FIX #1 & #4: Better TOAST UI initialization with custom options
    _initToastEditor(initialMarkdown = '') {
        ...
        try {
            this._toastEditor = new toastui.Editor({
                ...
                // FIX #1: Disable auto horizontal rule after headings
                autofocus: false,
                toolbarItems: [
                ...
                // FIX #4: Custom events to clean up markdown
                events: {
                    change: () => {
    ...
    // FIX #4: Clean up markdown formatting issues
    _cleanMarkdown(md) {
        return md
            ...
            .replace(/\\\</g, '<')
            .replace(/\\\>/g, '>')
            // Remove extra horizontal rules between sections
            .replace(/\*\*\*\n\*\*\*\n/g, '\n\n')
    ...
    // FIX #3: Proper editor opening with post data
    _openEditor(post) {
    ...
    // FIX #2: Better post list handling
    _onPostList(data) {
...
console.log('✍️ MdxBlogEditor v6 (All Fixes Applied) registered');
