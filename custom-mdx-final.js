class MdxBlogEditor extends HTMLElement {
    ...
    constructor() {
        ...
        // Flag to track if widget is ready
    ...
    static get observedAttributes() {
        return ['post-list','upload-result','save-result','delete-result','notification','load-data','widget-ready'];
    }

    attributeChangedCallback(name, _, val) {
        if (!val) return;
        try {
            if (name === 'widget-ready') {
                this._widgetReady = true;
                // Now safe to request post list
    ...
    connectedCallback() {
        // Don't emit immediately - wait for widget to signal ready
        // Widget will set widget-ready attribute when initialized
    }
...
<div class="msec">
    ...
    _wire() {
        ...
        s.getElementById('toolbar').addEventListener('click', e => {
            ...
            if (b) this._toolbarInsert(b.dataset.ins);
        });
        ...
        // Fix: Trigger slug generation on every keystroke
        ...
        // Add "/" shortcut listener to blocks panel
        this._wireSlashShortcut();
    }
    ...
    // NEW: "/" shortcut to show block menu
    _wireSlashShortcut() {
        const s = this._root;
        const blocksPanel = s.getElementById('blocksPanel');
        
        blocksPanel.addEventListener('keydown', (e) => {
            if (e.key === '/' && !e.shiftKey && !e.ctrlKey && !e.metaKey && !e.altKey) {
                const focused = this._root.activeElement;
                const wrap = focused?.closest?.('[data-block-id]');
                
                // Only trigger if we're in a contenteditable that's empty
                if (focused && focused.classList.contains('ce') && focused.textContent.trim() === '') {
                    e.preventDefault();
                    
                    const blockId = wrap ? parseInt(wrap.getAttribute('data-block-id')) : null;
                    const idx = blockId ? this._blocks.findIndex(b => b.id === blockId) : this._blocks.length - 1;
                    
                    // Show popup near the current block
                    const popup = s.getElementById('bmPopup');
                    const rect = wrap ? wrap.getBoundingClientRect() : { bottom: 100, left: 100 };
                    popup.style.top = (rect.bottom + 4) + 'px';
                    popup.style.left = rect.left + 'px';
                    popup.dataset.after = idx;
...
<input class="task-txt js-tt" type="text" value="${(it.text||'').replace(/"/g,'&quot;')}" placeholder="Task…" data-ii="${ii}">
    ...
    /* FIX: Toolbar now properly converts focused block type */
    _toolbarInsert(type) {
        const focused = this._root.activeElement;
        const wrap = focused?.closest?.('[data-block-id]');
        ...
        // If no block is focused, add new one at end
        this._addBlock(type);
    }

    /* FIX: Improved drag & drop with proper event binding */
    _bindDrag(wrap, idx) {
        const s = this._root;
        ...
        wrap.addEventListener('dragend', () => {
            ...
            s.querySelectorAll('.bw').forEach(w => w.classList.remove('drag-over'));
            this._dragIdx = null;
        });
        ...
        wrap.addEventListener('drop', e => {
            ...
            const targetIdx = this._dragIdx < idx ? idx - 1 : idx;
...
<svg class="spin-anim" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="20" height="20">
    ...
    _toMarkdown() {
        return this._blocks.map(b => {
            ...
            switch (b.type) {
                ...
                case 'quote': return t.split('\n').map(line => `> ${line}`).join('\n');
                case 'nested-quote': return t.split('\n').map(line => `>> ${line}`).join('\n');
                ...
                case 'image': 
                    return `![${b.data.alt || ''}](${b.data.src || ''})`;
                
                case 'hr': 
                    return `---`;
                
                case 'escape': 
                    // Properly escape markdown characters
                ...
                case 'table': {
                    ...
                    const header = `| ${r[0].join(' | ')} |`;
                    const separator = `| ${r[0].map(() => '---').join(' | ')} |`;
                    const rows = r.slice(1).map(row => `| ${row.join(' | ')} |`).join('\n');
    ...
    _mdToHtml(md) {
        ...
        // Code blocks first (to protect them)
        ...
        html = html.replace(/^(?!<[h1-6ul ptd]|<block|<hr|<pre|<table|<tr)(.+)$/gm, '<p>$1</p>');
    ...
    // ═════════════════════════════════════════════════════════
    // LOAD EXISTING POST INTO EDITOR (IMPROVED)
    // ═════════════════════════════════════════════════════════
    ...
    _parseMd(md) {
        ...
        while (i < lines.length) {
            ...
            if (line.startsWith('```')) {
                ...
                this._blocks.push({ id, type: 'code-block', data: { lang, code: code.trimEnd() } });
                i++; // Skip closing ```
                continue;
            }
            ...
            // Headers
            if (line.startsWith('###### ')) {
                this._blocks.push({ id, type: 'h6', data: { text: line.slice(7) } });
            } else if (line.startsWith('##### ')) {
                this._blocks.push({ id, type: 'h5', data: { text: line.slice(6) } });
            } else if (line.startsWith('#### ')) {
                this._blocks.push({ id, type: 'h4', data: { text: line.slice(5) } });
            } else if (line.startsWith('### ')) {
                this._blocks.push({ id, type: 'h3', data: { text: line.slice(4) } });
            } else if (line.startsWith('## ')) {
                this._blocks.push({ id, type: 'h2', data: { text: line.slice(3) } });
            } else if (line.startsWith('# ')) {
                this._blocks.push({ id, type: 'h1', data: { text: line.slice(2) } });
            }
            // Nested quote
            else if (line.startsWith('>> ')) {
                this._blocks.push({ id, type: 'nested-quote', data: { text: line.slice(3) } });
            }
            // Quote
            else if (line.startsWith('> ')) {
                this._blocks.push({ id, type: 'quote', data: { text: line.slice(2) } });
            }
            // HR
            else if (line.trim() === '---') {
                this._blocks.push({ id, type: 'hr', data: {} });
            }
            // Task list
            else if (line.match(/^- \[(x| )\] /)) {
                const checked = line.includes('[x]');
                const text = line.replace(/^- \[(x| )\] /, '');
                this._blocks.push({ id, type: 'tasklist', data: { items: [{ text, checked }] } });
            }
            // Unordered list
            else if (line.startsWith('- ')) {
                this._blocks.push({ id, type: 'ul', data: { text: line.slice(2) } });
            }
            // Ordered list
            else if (line.match(/^\d+\. /)) {
                this._blocks.push({ id, type: 'ol', data: { text: line.replace(/^\d+\. /, '') } });
            }
            // Image
            else if (line.match(/^!\[([^\]]*)\]\(([^)]+)\)/)) {
                ...
                this._blocks.push({ 
                    id, 
                    type: 'image', 
                    data: { alt: match[1], src: match[2], pendingFile: null, preview: null } 
                });
            }
            // Link
            else if (line.match(/^\[([^\]]+)\]\(([^)]+)\)$/)) {
                const match = line.match(/^\[([^\]]+)\]\(([^)]+)\)$/);
                this._blocks.push({ id, type: 'link', data: { text: match[1], url: match[2] } });
            }
            // Inline code
            else if (line.match(/^`[^`]+`$/)) {
                this._blocks.push({ id, type: 'inline-code', data: { text: line.slice(1, -1) } });
            }
            // Table (starts with |)
            else if (line.startsWith('|')) {
                // Parse table - skip for now, just add as paragraph
                this._blocks.push({ id, type: 'paragraph', data: { text: line } });
            }
            // Default: paragraph
            else {
                this._blocks.push({ id, type: 'paragraph', data: { text: line } });
...
console.log('✍️ MdxBlogEditor v5 FIXED registered');
